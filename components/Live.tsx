...
import { useBroadcastEvent, useEventListener } from '@/assets/liveblocks.config';
...
function Live({ canvasRef }: Props) {
    ...
    const handlePointerDown = useCallback((event: React.PointerEvent) => {
        ...
        setCursorState((state: CursorState) => cursorState.mode === CursorMode.Reaction ? { ...state, isPrsent: true } : state)
    ...
    return (
        <div id='canvas' onPointerMove={handlePointerMove}
            onPointerLeave={handlePointerLeave}
            onPointerDown={handlePointerDown}
            onPointerUp={handlePointerUp}
            className='border-2 green h-screen w-full'>
